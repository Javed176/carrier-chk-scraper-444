...
class DatabaseManager:
    ...
    def _ensure_session_state(self):
        """Safely initialize session state storage at runtime inside functions."""
        try:
            if "fallback_users" not in st.session_state:
                admin_user = st.secrets.get("ADMIN_USERNAME", "admin").strip() if hasattr(st, "secrets") else "admin"
                admin_pass = st.secrets.get("ADMIN_PASSWORD", "admin_secret_password_123") if hasattr(st, "secrets") else "admin_secret_password_123"
    ...
    def authenticate_and_login(self, username: str, password: str) -> Tuple[bool, str, Optional[Dict]]:
        ...
        # ── 1. Check Configured Secrets Admin Account ───────────────────────
        secrets_admin_u = st.secrets.get("ADMIN_USERNAME", "admin").strip() if hasattr(st, "secrets") else "admin"
        secrets_admin_p = st.secrets.get("ADMIN_PASSWORD", "admin_secret_password_123") if hasattr(st, "secrets") else "admin_secret_password_123"

        if username == secrets_admin_u and password == secrets_admin_p:
            user_info = {
                "username": username,
                "is_admin": True,
                "delay_ms": int(st.secrets.get("DEFAULT_DELAY_MS", 500)) if hasattr(st, "secrets") else 500,
                "session_duration_hours": float(st.secrets.get("DEFAULT_SESSION_HOURS", 3.0)) if hasattr(st, "secrets") else 3.0,
